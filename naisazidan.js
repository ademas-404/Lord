			...
			const getGroup = async function(totalchat){
				...
				if (authorname != undefined) { } else { authorname = groupName }	
					function addMetadata(packname, author) {	
						if (!packname) packname = 'Raisa'; if (!author) author = 'Bot';author = author.replace(/[^a-zA-Z0-9]/g, '');	
...
//========================================================================================================================//
		...
		/*const chats = ''
		if (chats.includes('p')) {
		 return reply(`Salam macam apa itu anjing`)
		}*/
 //let bodyy = Ofc
		const caption = Ofc
		const commands = caption || budy || ''
		const aurel = budy.split(' ')
		if (aurel.includes('p')) { //Edit sesuka hati kamu sayang
            reply(`salam macam apa itu anjing`)
        }
		if (aurel.includes('menu')) {
let	ihsh = `${owner_number}@s.whatsapp.net`
let cekvipp = ms(premium.getPremiumExpired(sender,premium) - Date.now())
let isimenu = `Hallo ${pushname} Berikut adalah daftar menu di bot ini:

1.Ketik *pay* untuk melihat List Pembayaran
2.Ketik *listvip* untuk melihat List Cheat VIP
3.Ketik *gantiserial* untuk melihat cara Ganti Serial
4.Ketik *download* untuk melihat cara download Injector Cheat 
5.Ketik *cashpb* untuk melihat List Top up PB
6.Ketik *setpb* untuk kode redeem set Indihome Point Blank
7.Ketik *gbpb* untuk list GB Point Blank
8.Ketik *prem* untuk Aplikasi Premium
9.Ketik *dmml* untuk melihat list Diamond Mobile Legend
10.Ketik *giveaway* untuk melihat infomasi tentang Giveaway
==============================
Ketik *.seller* Jika ingin order/perpanjang cheat 

Note :
Silahkan ketik kata kunci diatas untuk info lebih lanjut
...
let gaklo = [{
	...
	sendButLocation(from, isimenu, `By Lord Zoe Rexha Xavier ${namabot}™© | ${owner_name}`, thumbnail, gaklo, {contextInfo: { mentionedJid: [sender]}})
	}
		if (aurel.includes('gantiserial')) {
			reply(`
...
- Khusus Cheat PB VIP gunakan Memberkey yang VIP
		...
		if (aurel.includes('download')) {
			reply(`
Cara Download Injector Cheat
1. Masuk ke Web https://enjoyers.xyz/auth/member/login
2. Login menggunakan Memberkey
3. Klik Download dan akan muncul pilihan Injector
4. Lalu Klik Injector yang kalian Pilih
`)
		}
		if (aurel.includes('cashpb')) {
			reply(`
⚔ ====Cash PointBlank==== ⚔
    CASH PB VIA ID/VOUCHER :
  • CASH 1800   Rp  10.000
  • CASH 3600   Rp  20.000
  • CASH 9000   Rp  50.000
  • CASH 18000  Rp  98.000
  • CASH 36000  Rp 197.000
  • CASH 54000  Rp 285.000
  • CASH 90000  Rp 465.000
  Note:
  1. Proses 1-5 Menit.
  2. 100% LEGAL 
  3. Tanyakan Stock Terlebih Dahulu Sebelum Transfer`)
		}
		if (aurel.includes('setpb')) {
			let dqw = fs.readFileSync(`./image/setpb.jpeg`)
let listnya = `Hai ${[pushname]} Berikut list harganya:
` 
let gaklo = [{
	"buttonId": `owner`,
	"buttonText": {
...
sendButLocation(from, listnya, `KODE REDEEM SET INDIHOME PB :
    
• Pindad SS2 V5 Indihome 15 Hari
• Kriss S.V Indihome 15 Hari
• Tactilite T2 Indihome 15 Hari

1 ID Max 12 Set = 1 ID Max 180 Hari.
Beli 1 set dapat 3 weapon diatas sekaligus, weapon diatas RULES.

🔔HARGA Rp 25.000/SET
Note :
1. Via Voucher dan Wajib 1 Set
2. Voucher yang sama tidak bisa di redeem / dipakai 2x.
3. Harga sudah pas/net.
4. Nego?? Sorry Skip :D
5. Tanyakan Stock Terlebih Dahulu Sebelum Transfer
${namabot}™© |By ${owner_name}`, dqw, gaklo, {contextInfo: { mentionedJid: [sender]}})
		}
		if (aurel.includes('gbpb')) {
			reply(`
GB EXP POINT BLANK :
▶️ 1 JUTA EXP Rp 20.000
(Proses 3 hari)
▶️ 2 JUTA EXP Rp 40.000
(Proses 6 hari)
▶️ 3 JUTA EXP Rp 50.000
(Proses 9 hari)
▶️ 4 JUTA EXP Rp 70.000
(Proses 12 hari)
▶️ 5 JUTA EXP Rp 90.000
(Proses 15 hari)
...
3. Ketik *.Pay* untuk melakukan pembayaran.
`)
		}
		if (aurel.includes('prem')) {
			reply(`
YOUTUBE PREMIUM :
• 120 HARI PRIVATE  = Rp 20.000
==================
Canva Premium
• 30 HARI  = Rp 15.000
==================
SPORTIFY PREMIUM :
• 90 HARI FAMPLAN = Rp 45.000
• 90 HARI INDPLAN  = Rp 40.000
`)
		}
		if (aurel.includes('dmml')) {
			let poto = fs.readFileSync(`./image/ml.jpeg`)
let gaklo = [{
	"buttonId": `owner`,
	"buttonText": {
		"displayText": "KONTAK"
...
2. 100% LEGAL 
3. Tanyakan Stock Terlebih Dahulu Sebelum Transfer
${namabot}™© |By ${owner_name}`, poto, gaklo, {contextInfo: { mentionedJid: [sender]}})
		}
		if (aurel.includes('giveaway')) {
			let gif = fs.readFileSync(`./image/gif.jpg`)
let gaklo = [{
	"buttonId": `owner`,
	"buttonText": {
		"displayText": "SELLER"
...
let gipewe = `Jika ${pushname} Lupa kode GiveAway nya silahkan chatt seller yah Klik tombol seller dibawah ini untuk mendapatkan nomor seller.`
sendButLocation(from, gipewe, `
⚔️====GIVEAWAY====⚔️
Infomasi: 
Tanggal : 2-Desember-2021
Pukul : 20.00 WIB
Tempat : Live Discord
https://discord.gg/6ebGcvPm

Pengundian akan di lakukan sebanyak 3x
...
Note :
- Link Live Streaming aja di kirim di Hari tanggal 2-Desember-2021
- Pemenang WAJIB memiliki CODE GIVEAWAY dan masuk ke dalam grup MEMBER VIP BARBAR x ENJOYERS
Jika tidak memenuhi syarat akan Hangus
- Pemenang HANYA DAPAT 1X
- Jika Kodenya Disebut Harus respon Jika tidak hangus
- Peserta Wajib Nonton LIVE Discord
- Link Discord Akan dibagikan sebelum H-1 pelaksanaan
- Hadiah Sewaktu-waktu dapat berubah tanpa pemberitahuan
====TERIMA KASIH ATAS PARTISIPASINYA====
${namabot}™© |By ${owner_name}`, gif, gaklo, {contextInfo: { mentionedJid: [sender]}})
		}
		if (aurel.includes('listvip')) {
		reply(`
Hallo ${pushname} Berikuat adalah daftar cheat yang tersedia:
...
• Free Fire Nox ✅ Ready
• Free Fire MAX  ✅ Ready [NEW]
• PointBlank Zepetto  ✅ Ready
• PointBlank Private ✅ Ready
• Call Of Duty Garena ✅ Ready
• Call Of Duty Global ✅ Ready
• Call Of Duty Vietnam ✅ Ready 
• SauSage Man ✅ Ready [NEW]
• Lost Saga Exotic ✅ Ready
• Lost Saga Origin VIP✅ Ready
• Lost Saga Origin VVIP ✅ Ready
• Lost Saga Forever ✅ Ready
• Lost Saga Fusion ✅ Ready
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
☑️ Paket Personal : PB Zepetto & PB Private Server
⏩1 Hari : Rp. 10.000,-
⏩2 Hari : Rp. 15.000,-
...
➡️ 60 Days : Rp 175.000
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
☑️ Harga Personal Apk Mod PUBG Mobile Android
➡️ 3 Days : Rp 50.000
➡️ 7 Days : Rp 75.000
➡️ 14 Day : RP 100.000
...
⚠️Jika Top up di Alfamart⚠️
▶️ Dana Minimal Top up 50k (Free admin)
▶️ Gopay minimal Top up 20k (Biaya admin ± 2500) 
▶️ Link Aja Minimal Top up 20k (Biaya Admin ±1000)
Note : 
Biaya Admin akan bayar ke kasir Alfamart
		
Kirim Bukti Transaksinya ke seller yaa 😁✌️`)	
			}
		switch (command) {
//if (aurel.includes('pay')) {
	case 'pay':		
						...
						for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
							...
							ini_list.push({
								...
								hehe = await Zeeone.sendMessage(from, {
									...
									Zeeone.sendMessage(from, `Nih Kak Contact Creator Ku ✨`, text, {quoted: hehe})
									}
									break
 ...
 `)
						...
						case 'allmenu': case 'help': case 'menu':
									...
									let gaklo = [{
										...
										sendButLocation(from, listnya,`
Hello ${pushname}🤗 Saya ${namabot}
 ...
 ${simbolnya} ${prefix}towame
										Total hit : ${hitall}${enter}Hit today : ${hit_today.length}${enter}${enter}${namabot}™© | ${owner_name}`, thumbnail, gaklo, {contextInfo: { mentionedJid: [sender]}})
										break
...
`
						...
						case 'brainly':
									...
									break
									// case 'brain':{
									// 	const brain = require('./lib/brainly')
									// 	if(!q) return reply('Mau cari apa lord')
									// brain(`${q}`).then(res => {
									// 	teks = `─ 「 *BRAINLY* 」 ─\n\n*Hasil Pencarian Dari ${q}*\n\n`
									// 	for (let Y of res.data) {
									// 		teks += `🔖 *PERTANYAAN :* ${Y.pertanyaan}\n💬 *JAWABAN :* ${Y.jawaban.judulJawaban}\n\n`
									// 		}
									// 		Zeeone.sendMessage(from, teks, {quoted: Ofc, detectLinks: false})
									// 	})
									// }
									// break
									/*case 'brain':
										if(isReg(obj)) return
											...
											if (args.length >= 2){
												...
												let tanya = budy.slice(5)
												let jum = Number(tanya.split('.')[1]) || 2
												if (jum > 10) return reply(from, 'Max 10!')
...
Alasan : ${reason}`, [sender], true)
						...
						case 'donasi':
									reply(`
Dana: 081310253704
Setelah melakukan transaksi kirim screenshoot ke owner sebagai bukti
`)
									break
